...
shapiro.test(mayfly$speed)      # p-value = 0.2572
shapiro.test(mayfly$abundance)  # p-value = 0.1046

# Significativo, p-value = 0.008393
...
corrplot(cor_matrix, method = "circle", type = "upper",
         tl.cex = 0.8, col = colorRampPalette(c("blue", "white","red"))(200))
...
plot(anscombe_data$x1, anscombe_data$y1,
     main="Conjunto1")
abline(lm(y1 ~ x1, data = anscombe_data), col = "green")

plot(anscombe_data$x2, anscombe_data$y2,
     main = "Conjunto2")
abline(lm(y2 ~ x2, data = anscombe_data), col = "orange")

plot(anscombe_data$x3, anscombe_data$y3,
     main = "Conjunto3")
abline(lm(y3 ~ x3, data = anscombe_data), col = "blue")
